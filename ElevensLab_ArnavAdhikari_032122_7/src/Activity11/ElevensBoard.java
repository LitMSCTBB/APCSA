...
public class ElevensBoard extends Board {
	...
	private List<Integer> findJQK(List<Integer> selectedCards) {
		...
		boolean foundJack = false; int indJack = -1;
		boolean foundQueen = false; int indQueen = -1;
		boolean foundKing = false; int indKing = -1;
		for (Integer kObj : selectedCards) {
			int k = kObj.intValue();
			if (cardAt(k).rank().equals("jack")) {
				foundJack = true; indJack = k;
			} else if (cardAt(k).rank().equals("queen")) {
				foundQueen = true; indQueen = k;
			} else if (cardAt(k).rank().equals("king")) {
				foundKing = true; indKing = k;
			}
		}
		if (foundJack && foundQueen && foundKing) return Arrays.asList(indJack, indQueen, indKing);
	...
	public boolean playIfPossible() {
		...
		boolean ret1 = playPairSum11IfPossible(); boolean ret2 = playJQKIfPossible();
		return ret1 || ret2;
	}
	...
	private boolean playPairSum11IfPossible() {
		/* *** TO BE IMPLEMENTED IN ACTIVITY 11 *** */
		List<Integer> find = findPairSum11(cardIndexes());
		if (find.size() > 0) { replaceSelectedCards(find); return true; }
		return false;
	}
	...
	private boolean playJQKIfPossible() {
		/* *** TO BE IMPLEMENTED IN ACTIVITY 11 *** */
		List<Integer> find = findJQK(cardIndexes());
		if (find.size() > 0) { replaceSelectedCards(find); return true; }
		return false;
	}
}
